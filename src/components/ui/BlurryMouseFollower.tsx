...
function BlurryMouseFollower() {
  ...
  return (
    <div className="blurry-mouse-follower-wrp absolute desktop" style={{ height: bodyHeight }}>
      <div className={`blurry-bg-element blurry-mouse-follower absolute ${isMouseHovering ? "blurry-mouse-follower-hovering" : ""}`} style={{ left: mousePos.x - (size / 2), top: mousePos.y - (size / 2), "--width": `${size}px` } as React.CSSProperties}></div>
    </div>
  )
}
